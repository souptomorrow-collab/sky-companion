...
// 取復刻花費資料：優先用「該場專屬樹」的實際復刻價(visit.items)；其次 ts-cost.json 手動覆寫；
// 都沒有才退回資料庫季節原價(season-original)。est=Wiki估算待校正。
function tsCostData(name, visit) {
  if (visit && visit.items && visit.items.length) return { items: visit.items, totals: visit.totals || {}, real: true, est: false };
...
function tsTreeHTML(name, visit) {
  const d = name ? tsCostData(name, visit) : null;
...
function tsItemsInline(name, visit) {
  const d = name ? tsCostData(name, visit) : null;
...
// 取某到達日的完整復刻場次（含該場專屬樹的實際復刻價 items/totals）
function tsVisitOn(dateStr) {
  const list = (typeof window !== 'undefined' && window.SKYDATA && window.SKYDATA.travelingSpirits) || [];
  for (const t of list) if (t.date === dateStr) return t;
  return null;
}
function renderSpirits(now) {
  ...
  if (now.getTime() < cur.departInst.getTime()) {
    ...
    const visit = tsVisitOn(dateKey(cur.cal));
  ...
  for (let i = -1; i <= 6; i++) {
    ...
    const visit = tsVisitOn(dateKey(t.cal));
    ...
    if (who) {
      ...
      list += `<details class="shard-day-x ts-cal">
        ...
        <div style="padding:6px 4px 2px">${tsItemsInline(who, visit)}</div>
      </details>`;
    } else {
