...
/* ---------- 雲端「逐項合併」同步 ----------
 * 集合型資料(物件 map：光之翼 wl、圖鑑 dex、任務勾選 quests_*、collToggle 等)→ 聯集，兩邊勾的都保留。
 * 純量/設定(字串、數字、布林、陣列)→ 偏好端(preferLocal=true 本機勝；false 雲端勝)。
 * 參數為 { appkey: 原始JSON字串值 }（即 localStorage 的原始值）。回傳同格式。
 */
function _isPlainObj(v) { return v && typeof v === 'object' && !Array.isArray(v); }
function _mergeMap(a, b, preferA) {
  const out = {}, keys = {};
  ...
  for (const k in keys) {
    ...
    const av = a[k], bv = b[k];
    if (!inA) out[k] = bv;
    else if (!inB) out[k] = av;
    else if (av === bv) out[k] = av;
    else if (_isPlainObj(av) && _isPlainObj(bv)) out[k] = _mergeMap(av, bv, preferA); // 巢狀 map 也聯集
    else if (av && !bv) out[k] = av;       // 偏好「有值/已勾選」
    else if (bv && !av) out[k] = bv;
    else out[k] = preferA ? av : bv;       // 兩邊都有值且不同 → 偏好端
  }
  return out;
}
function mergeProfileData(localData, remoteData, preferLocal) {
  localData = localData || {}; remoteData = remoteData || {};
  const out = {}, keys = {};
  for (const k in localData) keys[k] = 1;
  for (const k in remoteData) keys[k] = 1;
  for (const k in keys) {
    const ls = localData[k], rs = remoteData[k];
    if (ls == null) { out[k] = rs; continue; }
    if (rs == null) { out[k] = ls; continue; }
    if (ls === rs) { out[k] = ls; continue; }
    let lo, ro, lok = false, rok = false;
    try { lo = JSON.parse(ls); lok = true; } catch (e) {}
    try { ro = JSON.parse(rs); rok = true; } catch (e) {}
    if (lok && rok && _isPlainObj(lo) && _isPlainObj(ro)) out[k] = JSON.stringify(_mergeMap(lo, ro, preferLocal));
    else out[k] = preferLocal ? ls : rs; // 純量/陣列:偏好端勝
  }
  return out;
}
