...
function wikiWinged() {
  ...
  const polys = shapes.map(s => {
    ...
    return `<polygon points="${pts}" fill="${s.color}22" stroke="${s.color}" stroke-width="1.5"/>
      <text x="${lp[0]}" y="${lp[1]}" fill="${s.color}" font-size="9" text-anchor="middle" paint-order="stroke" stroke="#0b1026" stroke-width="2.4">${escapeHtml(s.short || s.name)}</text>`;
  }).join('');
  const dots = wls.map(w => {
    const dot = `<circle cx="${w.pos[0]}" cy="${w.pos[1]}" r="2.6" fill="#ffe9b0" stroke="#1a1326" stroke-width="0.5"><title>${escapeHtml('#' + w.order + ' ' + w.desc)}</title></circle>`;
  ...
  const list = Object.keys(byRealm).map(rk => `<details class="wiki-card">
    ...
    <div class="sp-body">${byRealm[rk].map(w => `<div class="dex-item"><span>#${w.order} ${escapeHtml(w.desc)}</span>${w.video ? `<a class="wiki-link" href="${w.video}" target="_blank" rel="noopener">影片↗</a>` : ''}</div>`).join('')}</div>
  </details>`).join('');
