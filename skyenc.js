...
const SD = (typeof window !== 'undefined' && window.SKYDATA) || { spirits: [], seasons: [], realms: [], events: [], travelingSpirits: [], currencies: [], shards: {}, daily: {} };
...
function dexMatch(sp) {
  ...
  if (dexState.q) {
    ...
    const hay = (sp.name + ' ' + (sp.season || '') + ' ' + (sp.realm || '')).toLowerCase();
    if (!hay.includes(q)) return false;
  }
  return true;
}
function spiritBadges(sp) {
  if (sp.type === 'Season' && sp.season) return `<span class="badge season-b">${escapeHtml(sp.season)}</span>`;
...
function wikiSeasons() {
  return SD.seasons.map(s => `<details class="wiki-card">
    <summary><b>${escapeHtml(s.name)}</b> <span class="muted">${s.start || ''} ~ ${s.end || ''}</span> <span class="badge none">${s.spirits.length} 位</span></summary>
    <div class="sp-body">${s.spirits.map(n => `<span class="pill">${escapeHtml(n)}</span>`).join(' ') || '<span class="muted">—</span>'}
...
function wikiRealms() {
  return SD.realms.map(r => `<div class="wiki-card open">
    <div class="wc-head"><b>${escapeHtml(r.name)}</b> ${r.wingedLight ? `<span class="badge none">✦ 光之翼 ${r.wingedLight}</span>` : ''}</div>
    <div class="sp-body">${r.areas.map(a => `<span class="pill">${escapeHtml(a)}</span>`).join(' ')}
...
function wikiEvents() {
  ...
  const rows = SD.events.map(e => {
    ...
    return `<details class="wiki-card"${next ? ' open' : ''}>
      <summary><b>${escapeHtml(e.name)}</b> ${e.recurring ? '<span class="badge season-b">每年</span>' : ''}</summary>
